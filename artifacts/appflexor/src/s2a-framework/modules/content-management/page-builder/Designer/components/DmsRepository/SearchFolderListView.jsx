    ...
    return (
        <div>
            {folderList.map(folder => {
                const item = folder?._source;
                const highlight = folder?.highlight;
                return (
                    <FolderView
                        key={item.id}
                        folder={item}
...
const FolderView = props => {
    ...
    const name = getTextByName("name", highlight, folder);
    const title = getTextByName("title", highlight, folder);
    const folder_type_name = getTextByName(
        "folder_type_name",
        highlight,
        folder,
    );
    const description = getTextByName("description", highlight, folder);
    const createdby = getTextByName("createdby", highlight, folder);
    const modifiedby = getTextByName("modifiedby", highlight, folder);
                    ...
                    <div className="flex-fill">
                        <div className="d-flex flex-column ">
                            <span
                                ...
                                <span
                                    title="Folder Name"
                                    className="fw-bold highlight-text">
                                    <Interweave content={name} />
                                    {/* {folder?.name}{" "} */}
                                </span>
                                {folder?.title && (
                                    <span
                                        className="ms-1 highlight-text"
                                        title="Folder Title">
                                        (
                            ...
                            <span className="">
                                Folder Type{": "}
                                <Interweave content={folder_type_name} />
                            </span>
                            <span>
                                Created{": "}
                                <Interweave
                                    className="highlight-text"
                                    content={formatDateTimeForUserView(
                                        folder?.datecreated,
                                    )}
                                />{" "}
                                by{" "}
                                <Interweave
                                    ...
                                    content={createdby}
                                />
                                {/* {formatDateTimeForUserView(folder?.datecreated)}{" "}
                                by {folder?.createdby} */}
                            </span>
                            <span>
                                Modified{": "}
...
export default SearchFolderListView;
...
function Tags({ items }) {
    ...
    return (
        ...
        parsedTags?.map(tag => (
            <Interweave
                key={tag?.name}
                className="highlight-text badge px-2 py-1"
                content={tag?.name?.replaceAll("_", " ")}
            />
            // <span
            //     key={tag?.name}
