...
const RenderPreview = ({ layout, components, mode }) => {
    ...
    useEffect(() => {
        if (!isEmpty(stateObj)) {
            ...
            if (isArrayEmpty(keys))
                setInfoMsg(
                    "Column required for validation are empty. This can lead to false validation results.",
                );
        }
    }, [stateObj]);
                ...
                if (validationSchema.required === "YES") {
                    try {
                        if (validationSchema.regex.length > 0) {
                            ...
                            let strToValidate =
                                objToValidate[validationSchema.db_column];
    ...
    function renderRow(row) {
        return (
            <Row
                ...
                mode={mode}
                handleInputFields={handleInputFields}></Row>
        );
    }
    ...
    return (
        <div className="container-fluid s2a-page-layout">
            {infoMsg !== "" && (
                <div
                    className="p-1 alert alert-info"
                    role="alert">
                    {infoMsg}
                </div>
            )}
