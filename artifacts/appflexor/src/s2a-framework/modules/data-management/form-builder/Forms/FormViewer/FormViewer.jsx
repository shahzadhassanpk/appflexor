...
const FormViewer = ({
        ...
        layout.map(row => {
            row.children.map(column => {
                column.children.map(component => {
                    ...
                    let value = evalDefault(foundComponent.data.value, type);
                    ...
                    switch (type) {
                        case "daterange":
                            ...
                            break;
                        case "subform":
                            //  if it is subform then its data will be kept in `subFormDefaultConfig` and not in `defalutFormData` which will become `formData`
    ...
    const checkValidation = (objToValidate, layout, components) => {
        ...
        for (let i = 0; i < schemaArr.length; i++) {
            ...
            if (objToValidate.hasOwnProperty(keyToValidate)) {
                if (
                    ...
                    try {
                        if (
                            ...
                            validationSchema.regex.length > 0
                        ) {
                            const regexExp = new RegExp(validationSchema.regex);
                            let strToValidate = "";

                            if (objToValidate[keyToValidate] !== undefined) {
                                strToValidate = objToValidate[keyToValidate];
                            }
                            ...
                            if (objToValidate[keyToValidate] !== undefined) {
                                strToValidate = objToValidate[keyToValidate];
                                if (typeof strToValidate === "number") {
                                    strToValidate = strToValidate.toString();
                                }
                            }
...
function formatDateTimeForDataBase(date) {
