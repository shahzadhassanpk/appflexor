...
function ProcessMonitor({ activeTab }) {
    ...
    const [camLogin, setCamLogin] = useState({});
    ...
    function getData() {
        var dataRequest = {
            dataKeys: [
                {
                    serviceParams: "tenant_process",
                    dataKey: "processList",
                    serviceKey: "pe.datalist",
                    mode: "formData",
                },
            ],
        };
        ...
        axios
            ...
            .then(response => {
                if (response.data.C_STATUS === "UNAUTHORIZED") {
                    ...
                    let _processList = {};
                    _processList.data = response.data.C_DATA["processList"][0];
                    setProcessList(_processList);
                    console.log("*********** processList");
                    console.log(response.data.C_DATA["processList"]);
                }
                setLoaded(true);
            })
            .catch(error => {
    ...
    function loginCamunda7(camLogin) {
        let username = camLogin.username;
        let password = camLogin.password;
        axios
            .post("/camunda/login", { username, password })
            .then(response => {
                if (response.status === 200) {
                    let data = response.data.data;
                }
    ...
    function ssoCamunda7() {
        const dataRequest = {};
        axios
            .post(BPM_API_URL + "?service.key=sso.camunda", dataRequest)
            .then(response => {
                if (response.status === 200) {
                    let camLogin = response.data.camLogin;
                    setCamLogin(camLogin);
                }
            })
            .catch(err => {
                console.error(err);
    ...
    //             {JSON.stringify(appContext.tenantSubscription)}
