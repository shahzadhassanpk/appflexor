...
// const DataListTableById = lazy(() =>
//     import("./DataListTableById"),
// );
...
export default function DataListViewer(props) {
    ...
    function applySelectedDataList(selectedDataListOnly) {
        try {
            if (selectedDataListOnly) {
                ...
                parsedLayout.actions.forEach(item => {
                    if (item.code === "pagination" && hidePagination) {
                        item.selected = false;
                    } else if (item.code === "search" && hideSearch) {
                        item.selected = false;
                    } else if (item.code === "allowall" && hideCheckBoxes) {
                        item.selected = false;
                    } else if (actions.includes(item.code) && hideActions) {
                        item.selected = false;
                    }
                });
                ...
                if (selectedDataListOnly?.form_id) {
                    getSelectedForm(selectedDataListOnly.form_id);
                }
            }
        } catch (error) {
            console.log(error);
    ...
    function getSelectedDataList(selectedItem) {
        // let cachedDataList = localStorage.getItem(
        //     `${tenantId}:${selectedItem.id}`,
        // );
        // if (cachedDataList) {
        //     let dataList = JSON.parse(cachedDataList);
        //     applySelectedDataList(dataList);
        //     return;
        // }
        var dataRequest = {};
                    ...
                    if (response.data.C_DATA) {
                        ...
                        if (response.data.C_DATA.selectedDataList.length > 0) {
                            if (selectedItem.id) {
                                ...
                                applySelectedDataList(selectedDataListOnly);
