...
const Favourits = ({
    ...
    useEffect(() => {
        if (
            ...
            // setFormKeys(formatFormKeys(repository));
            lastRootFoldersId.current = selectedParentFolder.id;
            lastSubFoldersId.current = null;
        }
            ...
            dataKeys: [
                {
                    ...
                    dataKey: "documents",
                    serviceKey: (parent?.id===repository.id?"dms.get.favorite.docs":"dms.get.docs"),
                    mode: "formData",
                },
                {
                    serviceParams: parent?.id,
                    dataKey: "folders",
                    serviceKey: (parent?.id===repository.id?"dms.get.favorite.folders":"dms.get.folders"),
                    mode: "formData",
                },
            ],
        };
        if (parent?.id===repository.id) {
                    ...
                    <div className="d-flex align-items-center justify-content-between p-2 action-header  s2a-border mb-2">
                        <div className="d-flex align-items-center  gap-3">
                            ...
                            <div className="search-count">
                                <span className="count pe-2" title="Folders">
                                    <i className="fas fa-folder"></i> {filteredFolders.length} :{" "}
                                    {folderList.length}
                                </span>
                                <span className="count pe-2" title="Documents">
                                    <i className="fas fa-file"></i> {filteredDocuments.length} :{" "}
                                    {documentList.length}
                                </span>
                            </div>
                            {/* <Upload /> */}
